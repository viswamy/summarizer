...
class TfIdf:
    ...
    def tf(self,blob):
        ...
        for word in blob.words:
            word = stemmer_test.get_stem(word)
            if word not in out:
                out[word] = 0
            out[word] += 1
        ...
        for i, blob in enumerate(self.testBloblist):
            ...
            for j in range(0,len(sentenceList)):
                ...
                for word in sentence.words:
                    word_stem = stemmer_test.get_stem(word)
                    if word_stem in self.wordDfDict:
                        tf = tfw[word_stem]
                        df = self.wordDfDict[word_stem]
...
t = TfIdf(corpusPath, 'tf_idf_stemmer_results' )
t.setup()
